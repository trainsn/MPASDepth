...
// 	cnpy::npy_save(equator_filename.c_str(), &equator[0], { (size_t)nCells }, "w");
    ...
    for (int layer_id = 25; layer_id >= 5; layer_id-=2)
	{
		...
		char npypath[1024];
		sprintf(npypath, "/fs/project/PAS0027/MPAS2/%s/%s/t%s/depth%d.npy", dir, fileid.c_str(), timeid, layer_id);
		...
		char imagepath[1024];
		sprintf(imagepath, "/fs/project/PAS0027/MPAS2/%s/%s/t%s/depth%d.png", dir, fileid.c_str(), timeid, layer_id);
